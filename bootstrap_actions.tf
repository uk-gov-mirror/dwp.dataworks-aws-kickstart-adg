resource "aws_s3_bucket_object" "metadata_script" {
  ...
  key        = "component/kickstart-analytical-dataset-generation/metadata.sh"
...
resource "aws_s3_bucket_object" "emr_setup_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/kickstart-analytical-dataset-generation/emr-setup.sh"
  content = templatefile("${path.module}/bootstrap_actions/emr-setup.sh",
    {
      VERSION                         = local.kickstart_adg_version[local.environment]
      ADG_LOG_LEVEL                   = local.kickstart_adg_log_level[local.environment]
      ...
      acm_cert_arn                    = aws_acm_certificate.kickstart-analytical-dataset-generator.arn
      ...
      cwa_log_group_name              = aws_cloudwatch_log_group.kickstart_analytical_dataset_generator.name
...
resource "aws_s3_bucket_object" "ssm_script" {
  bucket  = data.terraform_remote_state.common.outputs.config_bucket.id
  key     = "component/kickstart-analytical-dataset-generation/start_ssm.sh"
  content = file("${path.module}/bootstrap_actions/start_ssm.sh")
}

resource "aws_s3_bucket_object" "installer_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/kickstart-analytical-dataset-generation/installer.sh"
...
resource "aws_s3_bucket_object" "logging_script" {
  bucket  = data.terraform_remote_state.common.outputs.config_bucket.id
  key     = "component/kickstart-analytical-dataset-generation/logging.sh"
  content = file("${path.module}/bootstrap_actions/logging.sh")
}

resource "aws_cloudwatch_log_group" "kickstart_analytical_dataset_generator" {
  name              = local.cw_agent_log_group_name
  retention_in_days = 180
  tags              = local.common_tags
}

resource "aws_cloudwatch_log_group" "kickstart_adg_cw_bootstrap_loggroup" {
  name              = local.cw_agent_bootstrap_loggrp_name
  retention_in_days = 180
  tags              = local.common_tags
}

resource "aws_cloudwatch_log_group" "kickstart_adg_cw_steps_loggroup" {
...
resource "aws_s3_bucket_object" "cloudwatch_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/kickstart-analytical-dataset-generation/cloudwatch.sh"
